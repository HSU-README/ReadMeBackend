package hsu.readme.exception;

public class ErrorMessage {
    public static final String NOT_VALIDATE_FORM = "형식이 갖춰지지 않았습니다";
    //SIGN UP
    public static final String EXISTED_USER_EMAIL = "존재하는 이메일입니다";
    public static final String EXISTED_USER_NAME = "존재하는 이름입니다";
    public static final String EMPTY_USER_UNIVERSITY = "대학이 입력되지 않았습니다";
    public static final String EMPTY_USER_MAJOR = "전공이 입력되지 않았습니다";
    public static final String USER_NOT_EXIST = "존재하지 않는 유저입니다";
    public static final String USER_DOC_LIKE = "유저가 이미 좋아요를 눌렀습니다.";
    public static final String USER_NOT_DOC_LIKE = "유저는 좋아요를 누르지 않았습니다";

    public static final String RECRUIT_NOT_EXIST = "존재하지 않는 채용공고입니다";

    public static String WRONG_EMAIL = "존재하지 않는 이메일입니다";
    public static String WRONG_PASSWORD = "입력한 비밀번호와 다릅니다";
}